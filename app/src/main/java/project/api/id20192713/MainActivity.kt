...
import project.api.id20192713.api.ApiManager
import project.api.id20192713.api.ApiResponse
import project.api.id20192713.api.Item
import project.api.id20192713.databinding.ActivityMainBinding
...
//            }
    ...
    private fun loadData(page: Int = 1) {
        ...
        lifecycleScope.launch {
            try {
                //val data: ApiResponse = ApiManager.getService().getDrugList(pageNo = page)
                ...
                if(SEARCH_KEYWORD == "")
                {
                    ...
                    when(SEARCH_OPTION_INDEX) {
                        ...
                        2 -> data = ApiManager.getService().getDrugList(pageNo=page, entpName = SEARCH_KEYWORD)
                        else -> data = ApiManager.getService().getDrugList(pageNo=page, entpName = SEARCH_KEYWORD)
                    }
                }
                if(data.header.resultCode == "00") {
                    ...
                    Log.i(TAG, "API RESULT CODE ERROR : ${data.header.resultCode}")
                }
            } catch (e: Exception) {
...
//        ApiManager.getService().getDrugList().enqueue(object : Callback<ApiResponse> {
...
//                                    Log.i(TAG, "${item.entpName}/${item.itemName}")
//                            } else {
//                                Log.i(TAG, "Items None")
//                            }
//                        } else {
//                            Log.i(TAG, "API RESULT CODE ERROR : ${data.header.resultCode}")
//                        }
